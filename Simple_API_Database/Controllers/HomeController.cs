    ...
    public class HomeController : Controller
    {
        ...
        */
        ...
        public List<Loser> GetLosingSymb()
        {
            ...
            if (!loserList.Equals(""))
            {
                ...
                loser_all = loser_all.GetRange(0, 10);
            }
            return loser_all;
        }
        ...
        */
        ...
        public List<Gainer> GetGainers()
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                gainerList = response.Content.ReadAsStringAsync().GetAwaiter().GetResult();
            }

            // now, parse the Json strings as C# objects
            if (!gainerList.Equals(""))
            {
                // https://stackoverflow.com/a/46280739
                gainer_all = JsonConvert.DeserializeObject<List<Gainer>>(gainerList);
                gainer_all = gainer_all.GetRange(0, 5);
            }
            return gainer_all;
        }

        public IActionResult gainer()
        {
            //Set ViewBag variable first
            ViewBag.dbSuccessgainer = 0;
            List<Gainer> gainer_all = GetGainers();

            //Save gainer_all in TempData, so they do not have to be retrieved again
            TempData["gainer_all"] = JsonConvert.SerializeObject(gainer_all);
            return View(gainer_all);
        }

        /*
            The Type action calls the Gettype method that returns a list of gainer_all.
            This list of gainer_all is passed to the Type View.
        */

        public IActionResult GainerSymb()
        {
            //Set ViewBag variable first
            ViewBag.dbSuccessgainer = 0;
            List<Gainer> gainer_all = GetGainers();
        ...
        */
        ...
        public IActionResult PopulateGainerSymb()
        {
            // Retrieve the gainer_all that were saved in the symbols method
            List<Gainer> gainer_all = JsonConvert.DeserializeObject<List<Gainer>>(TempData["gainer_all"].ToString());
            foreach (Gainer gainer in gainer_all)
            {
                //Database will give PK constraint violation error when trying to insert record with existing PK.
                //So add gainer only if it doesnt exist, check existence using symbol (PK)
                if (dbContext.Gainer_all.Where(c => c.symbol.Equals(gainer.symbol)).Count() == 0)
                {
                    dbContext.Gainer_all.Add(gainer);
                }
            }
